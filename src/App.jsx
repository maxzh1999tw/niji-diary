...
function Icon({ name, size = 24 }) {
  ...
  if (name === 'cameraRainbow') return <svg {...common}><path d="M5 6.5 6.5 3h4l1.5 3.5h7.5A2.5 2.5 0 0 1 22 9v9.5a2.5 2.5 0 0 1-2.5 2.5h-15A2.5 2.5 0 0 1 2 18.5V9a2.5 2.5 0 0 1 2.5-2.5H5Z" fill="var(--surface)" /><circle cx="12" cy="13" r="4.5" fill="var(--cyan)" stroke="var(--pink)" strokeWidth="2.2" /><circle cx="12" cy="13" r="1.5" fill="var(--yellow)" stroke="var(--ink)" strokeWidth="1.4" /><circle cx="18.5" cy="10" r="1.2" fill="var(--yellow)" stroke="var(--ink)" strokeWidth="1" /></svg>
...
function CompletedPolaroid({ item, alt, lang, t, className = '', imageLoading = 'lazy', editable = false, onCaptionChange, onCaptionCommit }) {
  ...
  const caption = item.caption ?? t.defaultCaption
  return <PolaroidCard className={className} image={item.cardImage} alt={alt} media={<EnergyStrip photos={item.photos} samples={item.samples} labels={t.colors} />} photos={item.photos} samples={item.samples} labels={t.colors} date={item.date} lang={lang} filmId={item.filmId} layoutId={item.layoutId}>{editable ? <EditablePolaroidCaption value={caption} t={t} multiline={item.layoutId === MOSAIC_LAYOUT_ID} onChange={onCaptionChange} onCommit={onCaptionCommit} /> : <PolaroidCaption>{caption}</PolaroidCaption>}</PolaroidCard>
}
...
function SolidBackgroundPicker({ selectedColor, canPickFromPolaroid, t, onSelect, onStartEyedropper, onClose }) {
  ...
  return <div className="solid-picker-scrim" role="presentation">
    <button className="solid-picker-dismiss" type="button" tabIndex="-1" aria-label={t.close} onClick={onClose} />
    <section ref={dialogRef} className="solid-picker-dialog" role="dialog" aria-modal="true" aria-labelledby="solid-picker-title" tabIndex="-1">
...
function SettingsScreen({ lang, setLang, t, migrationEnabled, migrationState, onMigrate, onOpenInfo }) {
  ...
  return (
    <section className="settings-screen screen-enter" aria-labelledby="settings-title">
      ...
      <div className="about-sticker"><span>NIJI</span><b>拾色日記</b><small>v1.0.7 · Max Edition</small></div>
    </section>
  )
}
...
export default function App() {
  ...
  const [solidBackgroundColor, setSolidBackgroundColor] = useState(null)
  ...
  async function confirmColor() {
    ...
    const collectionComplete = COLOR_KEYS.every((key) => nextDay.photos[key])
    if (!collectionComplete) showMessage(formatText(t.colorAdded, { color: t.colors[COLOR_KEYS.indexOf(selectedColor)] }))
    else {
      clearTimeout(messageTimer.current)
      setMessage('')
    }
  ...
  function handleSolidBackground(color) {
    const normalized = normalizeSolidBackgroundColor(color)
    if (!normalized) return
    setBackground(createSolidBackgroundSource(normalized))
    setSolidBackgroundColor(normalized)
  ...
  async function finishRainbowCard(captionOverride) {
    ...
    try {
      ...
      const renderSource = {
        ...
        achievements: {
          ...(day.achievements ?? {}),
          filmChallenges: createFilmChallenges(completionSource, t.defaultCaption),
        },
      }
