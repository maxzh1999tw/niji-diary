...
assert.match(appSource, /function PolaroidCard\(\{[\s\S]*?photoInteraction = \{\}/, 'Polaroid cards must expose the photo interaction surface to the studio')
assert.match(appSource, /className=\{`polaroid-photo \$\{photoClassName\}`\} \{\.\.\.photoInteraction\}/, 'studio gestures must listen on the complete photo area')
assert.match(appSource, /const canPan = typeof event\.target\?\.closest === 'function' && Boolean\(event\.target\.closest\('\.rainbow-artwork'\)\)/, 'single-finger panning must require the rainbow target')
assert.match(appSource, /entries\.length === 1 && entries\[0\]\[1\]\.canPan/, 'a lone pointer may resume panning only when it began on the rainbow')
assert.match(appSource, /const photoInteraction = \{[\s\S]*?onPointerDown: beginGesture,[\s\S]*?onPointerMove: moveGesture,[\s\S]*?onPointerUp: endGesture/, 'pinch and rotation handlers must be attached to the full photo surface')
assert.match(appSource, /photoInteraction=\{photoInteraction\}/, 'Rainbow Studio must pass the full photo gesture handlers to the shared card')
assert.match(appSource, /function usePolaroidTypography\(geometry\)/, 'all Polaroid text layers must use one measured typography contract')
assert.match(appSource, /const observer = new ResizeObserver\(\(\[entry\]\) => updateScale\(entry\?\.contentRect\.width\)\)/, 'caption typography must recalculate from the rendered card width')
assert.match(appSource, /getPolaroidTypographyScale\(layout, measuredWidth, fontSize\)/, 'caption scaling must use the shared canonical typography helper')
assert.match(appSource, /<span ref=\{dateRef\} className="polaroid-date-text">/, 'date text must share the measured typography layer instead of relying on card-specific CSS')
...
assert.match(appSource, /onChange: \(event\) => onChange\(multiline \? clampCaptionLines\(event\.target\.value\) : event\.target\.value\)/, 'multiline caption changes must keep the four-line limit')
...
assert.match(appStyles, /\.composition-canvas \.polaroid-caption-input \{[^}]*border-bottom-color:/, 'studio caption inputs must reuse the shared caption typography instead of overriding its size')
...
assert.match(appStyles, /\.composition-canvas-photo \{[^}]*touch-action: none;/, 'the complete photo surface must keep browser gestures from cancelling pinch and rotation')
