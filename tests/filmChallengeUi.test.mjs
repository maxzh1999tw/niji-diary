...
assert.match(appStyles, /\.studio-layout-choices button \{[^}]*border-color: transparent;[^}]*background: transparent;[^}]*box-shadow: none;/, 'studio layout buttons must not draw a frame around the preview')
assert.match(appStyles, /\.studio-layout-choices button\.selected \{[^}]*border-color: transparent;[^}]*background: #fff4fb;[^}]*box-shadow: none;/, 'studio layout selection should use a quiet fill instead of a second frame')
...
assert.match(appStyles, /\.canvas-source-actions \{[^}]*top: 16px;[^}]*right: 18px;/, 'background actions must stay in the upper-right corner of the studio stage')
assert.match(appStyles, /@media \(max-width: 760px\)[\s\S]*?\.canvas-stage \{ padding: 12px 116px 10px 15px; \}[\s\S]*?\.canvas-source-actions \{ top: 12px; right: 15px; width: 86px; flex-direction: column; \}/, 'small screens must reserve a right-side rail for complete background-action labels')
assert.doesNotMatch(appStyles, /\.canvas-source-actions span \{ display: none;/, 'background-action labels must remain visible on small screens')
