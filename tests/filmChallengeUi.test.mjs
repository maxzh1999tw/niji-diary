...
assert.match(appSource, /function FilmLayoutSupport\([\s\S]*?role="group" aria-label=\{t\.layoutSupportedLabel\}[\s\S]*?getSupportedFilmLayoutIds/, 'film support thumbnails must keep an accessible group label without visible helper text')
...
assert.match(appStyles, /\.film-card-top \{[^}]*justify-content: flex-start;/, 'film previews must align to the left so support layouts can occupy the right side')
