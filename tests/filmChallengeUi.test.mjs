...
assert.match(appSource, /className=\{`archive-card \$\{getFilmLayoutId\(getFilm\(item\.filmId\), item\.layoutId\) === MOSAIC_LAYOUT_ID \? 'archive-card-mosaic' : ''\}`\}/, 'album mosaic cards must reserve enough width for all caption lines')
...
assert.match(appStyles, /\.archive-card-mosaic \{[^}]*grid-column: span 2;/, 'album mosaic cards must span two columns so four caption lines keep their shared geometry')
assert.match(appStyles, /@media \(max-width: 480px\)[\s\S]*?\.archive-grid \{ grid-template-columns: minmax\(0, 1fr\); \}[\s\S]*?\.archive-card-mosaic \{ grid-column: auto; \}/, 'narrow album layouts must switch mosaic cards to one full-width column')
