...
assert.match(appStyles, /@media \(max-width: 760px\)[\s\S]*?\.canvas-stage \{ padding: 12px 15px 10px; \}[\s\S]*?\.canvas-source-actions \{ top: 12px; right: 15px; width: auto; flex-direction: row; gap: 5px; \}/, 'small screens must keep the Polaroid centered while actions stay in a compact icon rail')
