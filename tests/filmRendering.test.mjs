...
assert.equal(mosaicLayout.caption.fontSize, 46, 'the mosaic caption must be large enough to fill the four-line area')
...
assert.equal(mosaicGeometry.captionCardStyle['--polaroid-caption-font-size'], '46px', 'the mosaic caption font must come from the canonical layout')
...
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-font-size'], '42px', 'the date font must come from the canonical layout')
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-line-height-px'], '42px', 'the date line height must come from the canonical layout')
assert.equal(classicGeometry.dateCardStyle['--polaroid-date-font-size'], '42px', 'the classic date font must use the same enlarged canonical value')
assert.equal(classicGeometry.dateCardStyle['--polaroid-date-line-height-px'], '42px', 'the classic date line height must use the same canonical value')
...
assert.ok(Math.abs(smallMosaicTypographyScale.cssScale - 6 / 23) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.visualScale - 0.6133333333333333) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.contentScale - 1 / 0.6133333333333333) < 0.000001, 'small previews must preserve canonical caption proportions while keeping the computed font at least 12px')
