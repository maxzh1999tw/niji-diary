...
assert.equal(mosaicLayout.caption.fontSize, 50, 'the mosaic caption must be large enough to fill the four-line area')
...
assert.equal(mosaicGeometry.captionCardStyle['--polaroid-caption-font-size'], '50px', 'the mosaic caption font must come from the canonical layout')
...
assert.ok(Math.abs(smallMosaicTypographyScale.cssScale - 6 / 25) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.visualScale - 2 / 3) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.contentScale - 1.5) < 0.000001, 'small previews must preserve canonical caption proportions while keeping the computed font at least 12px')
