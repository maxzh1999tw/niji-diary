...
assert.equal(POLAROID_LAYOUT.caption.fontSize, 54, 'the canonical caption size must be shared by every layout')
assert.equal(POLAROID_LAYOUT.date.x, 670, 'the date slot must begin close to the caption edge so its first character is not clipped')
assert.equal(POLAROID_LAYOUT.date.width, 280, 'the date slot must reserve enough width for the complete date')
assert.equal(POLAROID_LAYOUT.date.x - (POLAROID_LAYOUT.caption.x + POLAROID_LAYOUT.caption.width), 20, 'the classic caption and date must keep a small intentional gap')
assert.equal(POLAROID_LAYOUT.footer.dateWidth, POLAROID_LAYOUT.date.width, 'captionless migration must use the same date width as the visible layout')
...
assert.equal(mosaicLayout.caption.fontSize, 54, 'the mosaic caption must use the shared canonical size')
assert.equal(mosaicLayout.caption.lineHeight, 60, 'the mosaic caption must leave a measured gap before the lower dashed edge')
...
assert.equal(mosaicGeometry.captionCardStyle['--polaroid-caption-font-size'], '54px', 'the mosaic caption font must come from the canonical layout')
assert.equal(mosaicGeometry.captionCardStyle['--polaroid-caption-line-height-px'], '60px', 'the mosaic caption line height must come from the canonical layout')
...
assert.equal(classicGeometry.captionCardStyle['--polaroid-caption-line-height-px'], '52px', 'the classic caption line height must remain unchanged')
assert.equal(mosaicGeometry.dateCardStyle.left, '67cqw')
assert.equal(mosaicGeometry.dateCardStyle.width, '28cqw')
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-font-size'], '48px', 'the date font must come from the canonical layout')
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-line-height-px'], '48px', 'the date line height must come from the canonical layout')
assert.equal(classicGeometry.dateCardStyle.left, '67cqw')
assert.equal(classicGeometry.dateCardStyle.width, '28cqw')
assert.equal(classicGeometry.dateCardStyle['--polaroid-date-font-size'], '48px', 'the classic date font must use the same enlarged canonical value')
assert.equal(classicGeometry.dateCardStyle['--polaroid-date-line-height-px'], '48px', 'the classic date line height must use the same canonical value')
...
assert.ok(Math.abs(smallMosaicTypographyScale.cssScale - 2 / 9) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.visualScale - 0.72) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.contentScale - 25 / 18) < 0.000001, 'small previews must preserve canonical caption proportions while keeping the computed font at least 12px')
