...
assert.deepEqual(mosaicSources.slice(0, 3).map(({ x, y }) => [x, y]), [[42, 35], [352, 35], [662, 35]], 'the first three color photos must form one horizontal row')
assert.deepEqual(mosaicSources.slice(3).map(({ x, y }) => [x, y]), [[42, 342], [42, 638], [42, 934], [42, 1230]], 'the final four color photos must stack on the left')
...
assert.equal(mosaicSources[0].width, 270, 'all color-photo frames must use the requested 270px size')
assert.equal(mosaicSources[1].x - (mosaicSources[0].x + mosaicSources[0].width), 40, 'the horizontal color-photo gap must stay compact')
...
assert.deepEqual(mosaicGeometry.photoCardStyle, { left: '35.2cqw', top: '34.2cqw', width: '58cqw', height: '72.5cqw' })
assert.deepEqual(mosaicGeometry.photoThumbnailStyle, { left: '35.2%', top: '22.8%', width: '58%', height: '48.333333%' })
...
assert.equal(mosaicGeometry.captionCardStyle.left, '35.2cqw')
assert.equal(mosaicGeometry.captionCardStyle.top, '110cqw')
assert.deepEqual(mosaicGeometry.captionThumbnailStyle, { left: '35.2%', top: '73.333333%', width: '58%', height: '17.533333%' })
