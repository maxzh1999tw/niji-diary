...
assert.deepEqual(mosaicSources.slice(0, 3).map(({ x, y }) => [x, y]), [[156, 35], [432, 35], [708, 35]], 'the first three color photos must form one right-aligned horizontal row')
...
assert.ok(mosaicSources[0].x > mosaicSources[3].x, 'the right-aligned top row must leave space above the left column')
