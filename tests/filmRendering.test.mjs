...
assert.deepEqual(mosaicSources.slice(0, 3).map(({ x, y }) => [x, y]), [[42, 35], [357.3333, 35], [672.6667, 35]], 'the first three color photos must form one horizontal row')
assert.deepEqual(mosaicSources.slice(3).map(({ x, y }) => [x, y]), [[42, 342], [42, 595.625], [42, 849.25], [42, 1102.875]], 'the final four color photos must stack on the left')
...
assert.equal(mosaicSources[6].y + mosaicSources[6].height, mosaicLayout.mediaHeight, 'the left color column must extend to the bottom of the media area')
...
assert.ok(mosaicLayout.caption.x > 300 && mosaicLayout.caption.x + mosaicLayout.caption.width < mosaicLayout.date.x, 'the caption must move to the lower-right area while keeping a gap before the date')
...
assert.deepEqual(mosaicGeometry.photoCardStyle, { left: '35.73333cqw', top: '34.2cqw', width: '58cqw', height: '72.5cqw' })
assert.deepEqual(mosaicGeometry.photoThumbnailStyle, { left: '35.73333%', top: '22.8%', width: '58%', height: '48.333333%' })
...
assert.equal(mosaicGeometry.captionCardStyle.left, '35.73333cqw')
assert.equal(mosaicGeometry.captionCardStyle.top, '110cqw')
assert.deepEqual(mosaicGeometry.captionThumbnailStyle, { left: '35.73333%', top: '73.333333%', width: '35%', height: '17.533333%' })
