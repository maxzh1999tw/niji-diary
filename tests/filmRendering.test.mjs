...
assert.equal(mosaicLayout.caption.fontSize, 42, 'the mosaic caption must be large enough to fill the four-line area')
...
assert.equal(mosaicGeometry.captionCardStyle['--polaroid-caption-font-size'], '42px', 'the mosaic caption font must come from the canonical layout')
...
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-font-size'], '40px', 'the date font must come from the canonical layout')
assert.equal(mosaicGeometry.dateCardStyle['--polaroid-date-line-height-px'], '40px', 'the date line height must come from the canonical layout')
...
assert.ok(Math.abs(smallMosaicTypographyScale.cssScale - 2 / 7) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.visualScale - 0.56) < 0.000001)
assert.ok(Math.abs(smallMosaicTypographyScale.contentScale - 1 / 0.56) < 0.000001, 'small previews must preserve canonical caption proportions while keeping the computed font at least 12px')
