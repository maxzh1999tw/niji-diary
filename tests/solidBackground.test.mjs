...
assert.doesNotMatch(composeSource, /studio-next-step/)
assert.match(composeSource, /Icon name="cameraRainbow" size=\{52\}/)
assert.match(appSource, /name === 'cameraRainbow'/)
...
const eyedropperStyleStart = appStyles.indexOf('.polaroid-eyedropper-target {')
const eyedropperStyleEnd = appStyles.indexOf('.polaroid-eyedropper-target.is-busy', eyedropperStyleStart)
const eyedropperStyles = appStyles.slice(eyedropperStyleStart, eyedropperStyleEnd)
assert.match(eyedropperStyles, /background: transparent;/)
assert.doesNotMatch(eyedropperStyles.replace(/box-shadow:\s*none;/g, ''), /box-shadow:/)
assert.match(appSource, /id="compose-title" aria-live="polite"/)
